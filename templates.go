		...
		<main>
			...
			<section class="hero">
				<div class="hero-inner">
					<h1 class="hero-title">ReadItLater Go Analyzer</h1>
					<p class="hero-sub muted">Gib eine URL ein und erhalte eine schnelle Analyse der Seite, Medien und strukturierten Daten.</p>
					<form class="hero-form" method="get" action="/" aria-label="URL suchen">
						<input class="hero-input" type="text" id="url" name="url" value="{{.URL}}" placeholder="https://example.com" required aria-label="URL zum Analysieren">
				...
				{{if .Video}}
					{{if (hasPrefix .Video "blob:")}}
						<div class="media-card">
							...
							<p class="muted">blob:-URLs funktionieren nur im Browser-Kontext und können nicht direkt heruntergeladen werden.</p>
							<code>{{.Video}}</code>
						</div>
					{{else}}
						<div class="media-card video-card">
							<div class="media-label">Video</div>
							<video id="video" controls style="width:100%; height:auto; display:block;">
								<source src="{{.Video}}">
						...
						(function() {
							...
							if (videoSrc && videoSrc.endsWith('.m3u8')) {
								if (window.Hls && Hls.isSupported()) {
									...
									document.querySelector('.video-card').innerHTML = '<div class="muted">Dein Browser unterstützt dieses Videoformat nicht direkt.</div>';
								}
							} else {
